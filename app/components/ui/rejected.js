...
const RejectedBookings = () => {
  ...
  useEffect(() => {
    const fetchBookings = async () => {
      try {
        const response = await fetch('/api/bookings/rejected');
        if (!response.ok) {
          throw new Error('Failed to fetch rejected bookings');
        }
  ...
  if (loading) {
    return (
      <div className="flex justify-center items-center h-20">
        ...
        <span className="ml-3 text-red-500">Loading rejected bookings...</span>
      </div>
    );
  }
  ...
  return (
    <div className="overflow-x-auto">
      <h2 className="text-xl font-bold mb-4">Rejected Bookings</h2>
      <table className="min-w-full bg-white border border-gray-200">
        ...
        <tbody>
          {bookings.length > 0 ? (
            ...
            <tr>
              <td colSpan="5" className="text-center px-4 py-2 text-gray-500">No rejected bookings found</td>
            </tr>
          )}
        </tbody>
      </table>
